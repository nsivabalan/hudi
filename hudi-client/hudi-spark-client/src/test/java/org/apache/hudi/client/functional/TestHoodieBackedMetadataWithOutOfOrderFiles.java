 ...
 */
public class TestHoodieBackedMetadataWithOutOfOrderFiles extends HoodieBackedTableMetadata {
  ...
  public TestHoodieBackedMetadataWithOutOfOrderFiles(HoodieEngineContext engineContext,
    ...
    this.metadataMetaClient = HoodieTableMetaClient.builder().setConf(storage.getConf())
  ...
  private HoodieTableFileSystemView getFileSystemView(HoodieEngineContext engineContext, HoodieTableMetaClient metaClient) {
    ...
    if (timeline.empty()) {
      final HoodieInstant instant = new HoodieInstant(HoodieInstant.State.COMPLETED, HoodieTimeline.DELTA_COMMIT_ACTION,
          metaClient.createNewInstantTime(false), metaClient.getTimelineLayout().getInstantComparator().completionTimeOrderedComparator());
      timeline =  metaClient.getTimelineLayout().getTimelineFactory().createDefaultTimeline(Stream.of(instant), metaClient.getActiveTimeline());
    }
