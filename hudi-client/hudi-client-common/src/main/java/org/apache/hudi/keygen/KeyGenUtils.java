...
public class KeyGenUtils {
   ...
   * @throws HoodieException if unable to deduce the encoding format
   */
  public static boolean deduceComplexKeyEncodingFromData(HoodieTableMetaClient metaClient, String recordKeyFieldName) {
    ...
    if (completedTimeline.empty()) {
      throw new HoodieException("Cannot deduce complex key encoding: no completed commits found in table "
    ...
    try {
      ...
      throw new HoodieException("Cannot deduce complex key encoding: no base files with records found in table "
          + metaClient.getBasePath());
    } catch (IOException e) {
