  ...
  private Map<String, String> prepareExtraMetadata(List<String> savepointedTimestamps, Option<String> earliestCommitToNotArchive) {
    if (savepointedTimestamps.isEmpty() && !earliestCommitToNotArchive.isPresent()) {
      ...
      if (earliestCommitToNotArchive.isPresent()) {
        metadataWithSavepoints.put(EARLIEST_COMMIT_TO_NOT_ARCHIVE, earliestCommitToNotArchive.get());
      }
  ...
  protected Option<HoodieCleanerPlan> requestClean() {
    ...
    if (config.incrementalCleanerModeEnabled() && cleanerPlan.getEarliestInstantToRetain() != null && config.maxDurationToCreateEmptyCleanMs() > 0) {
      ...
      if (lastCleanInstant.isPresent())  {
        try {
          // Since we don't have requested time with us for clean planning at this point, lets take the latest completed ingestion write and compute the time from it.
          Option<HoodieInstant> latestWriteRequestedTime = table.getActiveTimeline().getWriteTimeline().lastInstant();
          if (latestWriteRequestedTime.isPresent()) {
            long currentCleanTimeMs = HoodieInstantTimeGenerator.parseDateFromInstantTime(latestWriteRequestedTime.get().requestedTime()).toInstant().toEpochMilli();
      ...
      if (eligibleForEmptyCleanCommit) {
        LOG.warn("Creating a dummy clean instant with earliestCommitToRetain of {}",
