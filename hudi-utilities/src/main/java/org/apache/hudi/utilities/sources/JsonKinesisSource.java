...
public class JsonKinesisSource extends KinesisSource<JavaRDD<String>> {
  ...
  private static final Logger LOG = LoggerFactory.getLogger(JsonKinesisSource.class);
  /** Shared instance — do not call mutating config methods; read ops (readTree, writeValueAsString) are thread-safe. */
  ...
  private static class ShardFetchSummary implements Serializable {
    ...
    ShardFetchSummary(String shardId, Option<String> lastSequenceNumber,
        Option<Long> lastArrivalTime, int recordCount, boolean reachedEndOfShard) {
    ...
    public Option<Long> getLastArrivalTime() {
  ...
  private static class ShardFetchResult implements Serializable {
    ...
    ShardFetchResult(List<String> records, ShardFetchSummary summary) {
    ...
    public ShardFetchSummary getSummary() {
      return summary;
    }
  }
