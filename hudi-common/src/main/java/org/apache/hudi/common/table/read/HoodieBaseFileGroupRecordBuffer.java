...
public abstract class HoodieBaseFileGroupRecordBuffer<T> implements HoodieFileGroupRecordBuffer<T> {
  ...
  protected Option<Schema> getSchemaOption(Map<String, Object> metadata) {
    ...
    if (existingRecordMetadataPair != null) {
      if (enablePartialMerging) {
        ...
        Option<Pair<HoodieRecord, Schema>> combinedRecordAndSchemaOpt = recordMerger.get().partialMerge(
            readerContext.constructHoodieRecord(Option.of(record), metadata, getSchemaOption(metadata)),
            ...
            readerContext.constructHoodieRecord(
                ...
                getSchemaOption(existingRecordMetadataPair.getValue())),
        ...
        if (combinedRecord.getData() != existingRecordMetadataPair.getLeft().get()) {
          // after partial merge, chances that the new schema is never seen before.
          ...
          default:
            ...
            if (payloadClass.isPresent()) {
              ...
              Option<Pair<HoodieRecord, Schema>> combinedRecordAndSchemaOpt = recordMerger.get().merge(
                  ...
                  readerContext.constructHoodieRecord(
                      existingRecordMetadataPair.getLeft(), existingRecordMetadataPair.getRight(),
                      getSchemaOption(existingRecordMetadataPair.getRight())),
                  schemaIdToSchemaMap.get(existingRecordMetadataPair.getRight().get(INTERNAL_META_SCHEMA_ID)),
                  props);
  ...
  protected Option<T> merge(Option<T> older, Map<String, Object> olderInfoMap,
    ...
    if (enablePartialMerging) {
      ...
      Option<Pair<HoodieRecord, Schema>> mergedRecord = recordMerger.get().partialMerge(
          readerContext.constructHoodieRecord(older, olderInfoMap, getSchemaOption(olderInfoMap)),
          schemaIdToSchemaMap.get(olderInfoMap.get(INTERNAL_META_SCHEMA_ID)),
          readerContext.constructHoodieRecord(newer, newerInfoMap, getSchemaOption(newerInfoMap)),
