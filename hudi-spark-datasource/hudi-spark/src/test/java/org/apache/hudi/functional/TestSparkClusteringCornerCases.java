...
public class TestSparkClusteringCornerCases extends HoodieClientTestBase {
  ...
  void testClusteringWithPendingIngestionInDifferentPartition(HoodieTableType tableType, boolean enableRowWriter) throws IOException {
    ...
    try (SparkRDDWriteClient client = getHoodieWriteClient(hoodieWriteConfig)) {
      ...
      String firstInstant = metaClient.createNewInstantTime(false);
      List<HoodieRecord> recordsPartition1 = dataGenPartition1.generateInserts(firstInstant, 5000);
      writeData(client, firstInstant, recordsPartition1);
      ...
      HoodieCommitMetadata commitMetadata = metaClient.getActiveTimeline().readCommitMetadata(hoodieInstant);
      ...
      String clusteringInstant = metaClient.createNewInstantTime(false);
      ...
      assertEquals(clusteringInstant, pendingClusteringInstants.get(0).requestedTime());

      // Step 3: Start ingestion into partition2 but leave it inflight
      // The ingestion instant will be greater than the clustering instant
      String secondInstant = metaClient.createNewInstantTime(false);
      ...
      WriteClientTestUtils.startCommitWithTime(client, secondInstant);
      List<WriteStatus> writeStatuses = client.insert(recordsRDD, secondInstant).collect();
      assertNoWriteErrors(writeStatuses);
      // In 1.x, complete the commit and then revert to inflight to simulate a pending ingestion
      client.commit(secondInstant, jsc.parallelize(writeStatuses, 1));
      metaClient.reloadActiveTimeline();
      HoodieInstant completedSecondInstant = metaClient.getActiveTimeline()
          .getCommitsTimeline().filterCompletedInstants()
          .filter(i -> i.requestedTime().equals(secondInstant))
          .firstInstant().get();
      metaClient.getActiveTimeline().revertToInflight(completedSecondInstant);
      ...
      assertEquals(clusteringInstant, completedClustering.get(0).requestedTime());
      ...
      assertEquals(secondInstant, stillPendingCommits.get(0).requestedTime());
      ...
      HoodieReplaceCommitMetadata replaceCommitMetadata = metaClient.getActiveTimeline().readReplaceCommitMetadata(
  ...
  private List<HoodieRecord> writeData(SparkRDDWriteClient client, String instant, List<HoodieRecord> recordList) {
    ...
    assertNoWriteErrors(writeStatuses);
    client.commit(instant, jsc.parallelize(writeStatuses, 1));
    return recordList;
  }
}
