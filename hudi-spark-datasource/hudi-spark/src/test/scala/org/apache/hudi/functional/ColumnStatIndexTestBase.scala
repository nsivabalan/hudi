...
class ColumnStatIndexTestBase extends HoodieSparkClientTestBase {
  ...
  protected def validateColumnStatsIndex(testCase: ColumnStatsTestCase,
    ...
    val indexedColumnswithMeta: Set[String] = HoodieTableMetadataUtil
      .getColumnsToIndex(metaClient.getTableConfig, metadataConfig, lazyOptTableSchema).asScala.toSet
    ...
    val (expectedColStatsSchema, _) = composeIndexSchema(sortedIndexedColumns.toSeq, indexedColumns, localSourceTableSchema)
