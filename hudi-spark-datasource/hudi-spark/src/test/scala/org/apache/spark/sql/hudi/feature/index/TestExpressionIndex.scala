...
class TestExpressionIndex extends HoodieSparkSqlTestBase with SparkAdapterSupport {
  ...
  test("Test Create Expression Index With Data Skipping") {
    withTempDir { tmp =>
      Seq("cow").foreach { tableType =>
        val tableName = generateTableName
        val basePath = s"${tmp.getCanonicalPath}/$tableName"
        var extraConf = if (HoodieSparkUtils.gteqSpark3_4) {
  ...
  test("Test Enable and Disable Expression Index") {
    withTempDir { tmp =>
      ...
      withSQLConf(HoodieMetadataConfig.EXPRESSION_INDEX_ENABLE_PROP.key() -> "false") {
      ...
      withSQLConf(HoodieMetadataConfig.EXPRESSION_INDEX_ENABLE_PROP.key -> "true") {
        // do another insert after initializing the index
        // a record with from_unixtime(ts, 'yyyy-MM-dd') = 2024-09-26
        spark.sql(s"insert into $tableName values(5, 10, 1727329324, 'a3')")
        // check query result for predicates including values when expression index was disabled
      ...
      Seq("cow", "mor").foreach { tableType =>
       ...
       """.stripMargin)
          ...
          withCompactionConfigs(tableType) {
            val extraConf = if (!isPartitioned) {
              Map(HoodieCompactionConfig.PARQUET_SMALL_FILE_LIMIT.key() -> "0")
            } else {
              Map.empty[String, String]
            }
            withSQLConf(extraConf.toSeq: _*) {
  ...
  test("Test expression index with inflight compaction") {
    withTempDir { tmp =>
      ...
      withSQLConf(HoodieCompactionConfig.PARQUET_SMALL_FILE_LIMIT.key() -> "0",
       ...
       """.stripMargin
        ...
        withSQLConf("hoodie.compact.line" -> "false",
        ...
        withSQLConf("hoodie.compact.inline" -> "false",
          ...
          withSQLConf(extraConf.toSeq: _*) {
           ...
           CREATE TABLE $tableName (
            ...
            val morConf = if (isTableMOR) {
              Map("hoodie.compact.inline" -> "true",
                "hoodie.compact.inline.max.delta.commits" -> "1")
            } else {
              Map.empty[String, String]
            }
            withSQLConf(morConf.toSeq: _*) {
      ...
      Seq("cow", "mor").foreach { tableType =>
       ...
       """.stripMargin)
          ...
          if (HoodieSparkUtils.gteqSpark3_4) {
            extraConf += ("spark.sql.defaultColumn.enabled" -> "false")
          }
  ...
  private def withCompactionConfigs[T](tableType: String)(f: => T): T = {
    val conf = if (tableType == "mor") {
      Map("hoodie.compact.inline" -> "true",
        "hoodie.compact.inline.max.delta.commits" -> "2")
    } else {
      Map("hoodie.compact.inline" -> "false")
    }
    withSQLConf(conf.toSeq: _*)(f)
  }
